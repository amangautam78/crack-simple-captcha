...
app = Flask(__name__)
cors = CORS(app, resources={r"/captcha/*": {"origins": "*"}})
...
def get_captcha():
    ...
    if response.status_code == 200:
        ...
        img_name = "out-img"+ str(img_save_count)+".png"
...
def process():
    ...
    print("This is result from image_to_text1 - ", result1)
    
    output = solving_text(result1)
    print('answer', output)


@app.route('/captcha/hello/')
def hello():
    return "captcha api working fine"

@app.route('/captcha/front/')
def front():
    return render_template('index.html')

@app.route('/captcha/text/')
def processing():
    result = 0
    img = get_captcha()
    #print('img', img)
    ...
    #print('bw_img', bw_img)
    img_text = image_to_text1(bw_img)
    #print('img_text', img_text)
    result = int(solving_text(img_text))
    return {'text':img_text[:len(img_text)-1], 'result':result}
